...
## Aanpak (`/aanpak`)
1. IKR-cijfers (125M / 263 / 400K) verifiëren in TikTok Business Center en `src/data/ikr-stats.ts` updaten.
2. Dedicated freelancer-foto (`card top.png`) als die beter is dan `freelancer-cta.jpg`.
_(Done aug 2026: scroll-hint gsm verdwijnt na eerste swipe/wheel; cutoff Bekijk-het-zelf; contact-CTA = teamfoto)_
...
## Cases (`/cases`)
1. **Tempus layout kiezen** — vergelijk `/cases/tempus` (A), `/cases/tempus/v2` (B), `/cases/tempus/v3` (C). Daarna de andere twee weg.
2. Foto's + tekst corrigeren (Casper levert assets/copy)
3. Betere hero/storyfoto's + logo's voor Maison Slash & Anneke (hero Maison Slash = nu videoframe)
4. Panos view-stat invullen
5. Echte food retainer-cases → laag 1 wanneer content klaar
_(Done aug 2026: Aïki/O'Tacos/Panos als lokale mp4 in `public/videos/food/`; speech-bubble tails overlap+in-flow; 3 Tempus-layouts)_
